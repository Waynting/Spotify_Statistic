      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            placeholder="搜尋專輯名稱..."
            className="w-full pl-10 pr-10 py-3 bg-gray-800 text-white rounded-lg border border-gray-700 focus:outline-none focus:border-spotify-green transition-colors text-sm sm:text-base min-h-[44px]"
          />
          {searchTerm && (
            <button
              onClick={() => setSearchTerm('')}
              className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-white min-w-[44px] min-h-[44px] flex items-center justify-center"
              aria-label="清除搜尋"
            >
              <X size={18} />
            </button>
        ...
        <button
          ...
          className={`btn btn-secondary flex items-center gap-2 relative min-h-[44px] px-4 py-3 ${
      ...
      {showFilters && (
        <div className="bg-gray-800 rounded-lg p-4 space-y-4 animate-in slide-in-from-top-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={handleReset}
              className="text-sm text-gray-400 hover:text-white min-h-[44px] px-3 py-2"
            >
              重設
            </button>
          </div>
