        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
          <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="flex flex-wrap gap-1" role="tablist" aria-label="時間範圍選擇">
                {TIME_WINDOWS.map((tw) => (
                  <button
                    key={tw.value}
                    onClick={() => setWindow(tw.value)}
                    className={`pill transition-all duration-200 min-h-[44px] px-3 py-2 ${window === tw.value ? 'active' : ''}`}
                    role="tab"
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="flex flex-wrap gap-1" role="radiogroup" aria-label="排序方式選擇">
                {sortOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => setSortBy(option.value)}
                    className={`pill transition-all duration-200 min-h-[44px] px-3 py-2 ${sortBy === option.value ? 'active' : ''}`}
                    role="radio"
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={isFetching}
                className="btn btn-secondary flex items-center gap-2 text-sm px-4 py-3 min-h-[44px] justify-center"
                aria-label="重新載入數據"
              >
                {isFetching ? (
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-1 w-48 bg-gray-800 rounded-lg shadow-lg border border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors flex items-center gap-2 rounded-t-lg min-h-[44px]"
                    >
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors flex items-center gap-2 min-h-[44px]"
                    >
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors flex items-center gap-2 rounded-b-lg min-h-[44px]"
                    >
        ...
        <div className="bg-gray-900 rounded-lg overflow-hidden" role="region" aria-label="專輯排行榜">
          ...
          <div className="divide-y divide-gray-800" role="list">
            {sortedAlbums.map((album, index) => (
              <div
                key={album.album_id}
                className="p-3 sm:p-4 md:p-6 hover:bg-gray-800/50 transition-all duration-200 group focus-within:bg-gray-800/50"
                role="listitem"
              >
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
                  {/* 排名 */}
                  <div className="flex-shrink-0 w-6 sm:w-8 md:w-12 text-center">
                    <span className="text-base sm:text-lg md:text-2xl font-bold text-gray-500 group-hover:text-gray-400 transition-colors" aria-label={`第 ${index + 1} 名`}>
                      #{index + 1}
                    </span>
                  </div>
                  
                  {/* 專輯封面 */}
                  <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-gray-800 rounded-lg overflow-hidden" aria-hidden="true">
                    {album.album_image ? (
                      <img 
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-white truncate text-sm sm:text-base md:text-lg" title={album.album_name}>
                      {album.album_name}
                    </h3>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 md:gap-4 text-xs md:text-sm text-gray-400 mt-1">
                  ...
                  {/* 播放按鈕 */}
                  <button 
                    className="flex-shrink-0 p-2 md:p-3 rounded-full bg-gray-800 hover:bg-spotify-green hover:text-black transition-all duration-200 opacity-0 group-hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-spotify-green focus:ring-offset-2 focus:ring-offset-gray-900 min-w-[44px] min-h-[44px] flex items-center justify-center"
                    aria-label={`播放 ${album.album_name}`}
                  >
                    <Play size={16} className="md:w-4 md:h-4" fill="currentColor" />
                  </button>
                </div>
              </div>
            ))}
